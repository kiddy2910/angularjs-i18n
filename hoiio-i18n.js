 ...
 */
    .provider('i18n', function(i18nLocaleContainerProvider) {
        ...
        return {
            ...
            setPreferredMessages: function(language, module) {
                i18nLocaleContainerProvider.setPreferredMessages(language, module);
            },
...
// Beware This is internal module, shouldn't use anywhere.
angular.module('i18n.localeContainer', [])
     ...
     *
     *  fixedOptions = [
     *      { language: 'en', requires: [ 'en.1', 'en.2' ] },
     *      { language: 'vi', requires: [ 'vi.1', 'vi.2' ] }
     *  ]
     *
     * @type {Array}
     */
    .provider('i18nLocaleContainer', function() {
        ...
        function getFixedOptionByLanguage(language) {
            for(var i=0; i<fixedOptions.length; i++) {
                if(fixedOptions[i].language === language) {
                    return fixedOptions[i];
                }
            }
            return null;
        }

        function getOrInitFixedOption(language) {
            var opt = getFixedOptionByLanguage(language);
            if(opt == null) {
                opt = {
                    language: language,
                    requires: []
        ...
        function isRequireExistInFixedOption(fixedOption, requireName) {
            for(var i=0; i<fixedOption.requires.length; i++) {
                if(fixedOption.requires[i] === requireName) {
                    return true;
                }
            }
            return false;
        ...
        return {
            ...
            /**
             * Set priority for getting message object
             *
             * @param language
             * @param requires
             */
            setPreferredMessages: function(language, requires) {
                var fixedOpt = getOrInitFixedOption(language);
                for(var i=0; i<requires.length; i++) {
                    if(!isRequireExistInFixedOption(fixedOpt, requires[i])) {
                        fixedOpt.requires.push(requires[i]);
                    }
                }
            },
            ...
            $get: function($window, $injector) {
                ...
                function getMessageObject(language, messageNamespace) {
                    var locale, fixedOpt, iterationRequire, message, i;
                    ...
                    fixedOpt = getFixedOptionByLanguage(language);

                    if(fixedOpt != null) {
                        // has priority
                        for(i=0; i<fixedOpt.requires.length; i++) {
                            iterationRequire = getRequireByName(locale, fixedOpt.requires[i]);
                            if(iterationRequire == null) {
                                throw 'You chose preferred message service [' + fixedOpt.requires[i] + "] doesn't exist.";
                            }
